...
def fail(message: str, code: int = 1) -> "NoReturn":
    print(f"[dev-automation] ERRO: {message}", file=sys.stderr, flush=True)
...
def ensure_github_auth(non_interactive: bool = False) -> str:
    ...
    if status.returncode != 0:
        ...
        out("[dev-automation] Nenhum usuário GitHub autenticado.")
        out("[dev-automation] Abrindo login oficial do GitHub CLI...")
        login = run(
...
def resolve_repo(project: str, mapping: dict[str, str], accessible: dict[str, list[tuple[str, str]]]) -> tuple[str, str]:
    ...
    if mapped:
        cleaned = mapped.removesuffix(".git")
        full_name = cleaned.split("github.com/", 1)[-1] if "github.com/" in cleaned else cleaned
        return full_name, mapped

    name = PurePosixPath(project).name
    matches = accessible.get(name, [])
    if not matches:
        fail(f"repositório não encontrado para '{project}' (nome GitHub esperado: {name})")
    if len(matches) > 1:
        names = ", ".join(full for full, _ in matches)
        fail(
            f"mais de um repositório acessível chamado '{name}': {names}. "
...
def sync(args: argparse.Namespace) -> int:
    ...
    out(f"[dev-automation] Projetos ativos: {len(projects)} ({len(roots)} repositórios + {len(children)} subprojetos)")
    ...
    for project in roots:
        ...
        try:
            full_name, clone_url = resolve_repo(project, mapping, accessible)
        except SystemExit:
            errors += 1
            if args.keep_going:
                continue
