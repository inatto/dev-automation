...
set -euo pipefail
ROOT="$1"; ENV="$ROOT/apps/web/config/production/app.env"; PORT="$(sed -n 's/^APP_PORT=//p' "$ENV")"; SERVICE="$(sed -n 's/^WEB_SYSTEMD_SERVICE=//p' "$ENV")"; sudo systemctl restart "$SERVICE"
for _ in {1..40}; do curl -fsS --max-time 2 "http://127.0.0.1:${PORT}/" >/dev/null 2>&1 && exit 0; sleep 1; done
