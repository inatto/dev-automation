...
# Usa a fonte canônica de projetos, preservando a ordem. Entradas *.zip são
# agregadores do Dev Automation e não são projetos da IDE. Subprojetos dentro
# de apps/ também não são abertos separadamente quando o projeto-pai está ativo.
configured_projects=()
...
while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
  ...
  # Entradas *.zip são agregadores de backup/importação (ex.: Code.zip,
  # orgs/orbital.zip), não projetos que a IDE deve abrir.
  [[ "${line,,}" == *.zip ]] && continue
...
for line in "${configured_projects[@]}"; do
  # Se o próprio projeto-pai também está configurado, não abre novamente um
  # projeto contido em <pai>/apps/<subprojeto>. O pai já cobre esse código na IDE.
  candidate="$line"
  skip_nested_app=0
  while [[ "$candidate" == */apps/* ]]; do
    parent="${candidate%/apps/*}"
    if [[ -n "${configured_set["$parent"]:-}" ]]; then
      skip_nested_app=1
      break
    fi
    candidate="$parent"
