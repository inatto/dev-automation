...
handle_watch_event() {
  ...
  # VERSION e o temporário atômico .VERSION-* são gerados pela própria rodada
  # de backup. Ignorar ambos evita que o incremento da versão se retroalimente
  # e dispare backups em loop.
  if [ "$event_path" = "$PROJECT_ROOT/VERSION" ] || [[ "$event_path" == "$PROJECT_ROOT/.VERSION-"* ]]; then
    return 0
  fi
