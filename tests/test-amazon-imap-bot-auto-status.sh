...
wait_status() {
  ...
  for i in $(seq 1 100); do
    output="$(HOME="$HOME_DIR" CODE_ROOT="$CODE_ROOT" PROJECTS_FILE="$TMP/projects" \
      ...
      bash "$ROOT/scripts/amazon-imap-bot/amazon-imap-bot-auto-status.sh" "Orbital Legal")"
...
HOME="$HOME_DIR" AUTO_CODE_STATE_DIR="$STATE_DIR" DEV_AUTOMATION_SKIP_CLEAR=1 \
  ...
  bash "$ROOT/scripts/project/project-command.sh" orbital-legal-auto "$PROJECT" local \
  >"$TMP/local.log" 2>&1 &
LOCAL_PID=$!
...
HOME="$HOME_DIR" AUTO_CODE_STATE_DIR="$STATE_DIR" DEV_AUTOMATION_SKIP_CLEAR=1 \
  DEV_AUTOMATION_ERROR_SOUND_ENABLED=0 \
  bash "$ROOT/scripts/project/project-command.sh" remote-orbital-legal-auto "$PROJECT" remote \
  >"$TMP/remote.log" 2>&1 &
REMOTE_PID=$!
