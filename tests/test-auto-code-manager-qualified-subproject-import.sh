...
PATH="$FAKE_BIN:$PATH" CODE_ROOT="$CODE_ROOT" DEV_MANAGER_PROJECTS_FILE="$TEST_PROJECT/config/projects/default.projects" \
  "$TEST_PROJECT/scripts/dev-manager/auto-code-manager.sh" --import-one "$INBOX/exec-agent-incremental.zip" >/dev/null
...
PATH="$FAKE_BIN:$PATH" CODE_ROOT="$CODE_ROOT" DEV_MANAGER_PROJECTS_FILE="$TEST_PROJECT/config/projects/default.projects" \
  "$TEST_PROJECT/scripts/dev-manager/auto-code-manager.sh" --import-one "$INBOX/dev-automation-exec-agent.zip" >/dev/null
