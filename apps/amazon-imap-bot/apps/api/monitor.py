...
class Monitor:
    ...
    def stop(self) -> None:
        if self.stop_event.is_set():
            return
        self.stop_event.set()
        aborted = self.mailbox.abort_all()
        self.on_event(
            f"{datetime.now().strftime('%H:%M:%S')} [SYSTEM] encerramento solicitado; "
            f"conexões IMAP interrompidas={aborted}"
        )
    ...
    def validate_reply_generation(self, row: dict, *, manual_force: bool = False) -> dict:
        """Valida se uma resposta pode ser gerada/refeita.

        O fluxo automático permanece idempotente e não duplica resposta enviada.
        No fluxo manual explícito, uma resposta já SENT pode originar uma NOVA
        resposta para o mesmo e-mail original, quantas vezes o operador quiser.
        """
        ...
        if outbound is not None:
            ...
            if current_status in {"send-queued", "sending"}:
                labels = {
                    "send-queued": "já está na fila de envio",
                    "sending": "já está sendo enviada",
                }
                raise RuntimeError(
                    f"esta mensagem {labels[current_status]}; aguarde o envio terminar antes de gerar outra"
                )
            if current_status == "sent":
                if manual_force:
                    ...
                    raise RuntimeError(
                        "esta mensagem já foi enviada; o fluxo automático não pode gerar resposta duplicada"
                    )
    ...
    def generate_or_regenerate_reply(
        ...
        *,
        ...
        with self.reply_generation_lock:
            # Revalida dentro do lock para eliminar corrida entre a validação HTTP
            # e o início efetivo da geração.
            validated = self.validate_reply_generation(row, manual_force=manual_force)
            inbound = validated["inbound"]
            outbound = validated["outbound"]
            account = validated["account"]
            create_new = bool(validated.get("create_new"))
            ...
            kind = "reply-followup" if create_new else ("reply-rewrite" if outbound else "reply-compose")
            request_summary = (
            ...
            try:
                ...
                if outbound is None or create_new:
                    previous_outbound_id = int((outbound or {}).get("id") or 0)
                    ...
                    if create_new:
                        self._event(
                            f"RESPOSTA MANUAL ADICIONAL criada resposta_id={outbound_id} "
                            f"após resposta_enviada_id={previous_outbound_id}",
                            "EMAIL",
                            "INFO",
                        )
                else:
    ...
    def run_account_once(self, account: Account) -> None:
        ...
        try:
            ...
            if self.stop_event.is_set():
                return
            conn = self.mailbox.connect(
                account, folder=self.settings.imap_folder, readonly=True
            )
            ...
            for uid in uids:
                if self.stop_event.is_set():
                    self._event(f"Sincronização de {account.email} interrompida pelo encerramento", "IMAP", "WARN")
                    break
