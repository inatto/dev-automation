...
- mantém o script real versionado em `dev-automation/scripts/dev-manager.sh`.
...
```bash
create_first_window "automation" "$HOME/Code/bots/dev-automation" "bash ./auto-code-manager.sh"
...
O arquivo `config/auto-code-manager.projects` aceita caminhos relativos a `/home/daniel/Code`.
...
- `orgs/station-app` gera `/home/daniel/Code/station-app.zip`.
- Ao compactar uma pasta agrupadora, todos os arquivos `config/auto-code-manager.ignore-zip` encontrados dentro dela são aplicados no respectivo subdiretório.
- Na importação, todos os arquivos `config/auto-code-manager.ignore-unzip` já existentes no destino também são aplicados recursivamente.
