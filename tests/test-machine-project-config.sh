...
source "$ROOT/scripts/core/project-config.sh"
...
DEV_MACHINE_ID="$MACHINE_ID" PROJECTS_FILE="$TMP/explicit.projects" bash -c 'source "$1/scripts/core/project-config.sh"; printf "%s\n" "${PROJECTS_FILE:-$(dev_projects_file "$2")}"' _ "$ROOT" "$TMP" > "$TMP/out"
