...
def _project_root() -> Path:
    # apps/amazon-imap-bot/apps/api/config.py -> apps/amazon-imap-bot
    return Path(__file__).resolve().parents[2]


def _discover_config_root(project_root: Path) -> Path:
    explicit = str(os.environ.get("AMAZON_IMAP_BOT_CONFIG_ROOT", "")).strip()
    if explicit:
        return Path(explicit).expanduser()
    # Localmente o subprojeto vive em dev-automation/apps/amazon-imap-bot,
    # enquanto a configuração compartilhada fica em dev-automation/.config.
    for parent in (project_root, *project_root.parents):
        candidate = parent / ".config" / "amazon-imap-bot"
        if candidate.is_dir():
            return candidate
    return project_root / ".config" / "amazon-imap-bot"


def _overlay(prefixes: tuple[str, ...], values: dict[str, str]) -> dict[str, str]:
    merged = dict(values)
...
def load_settings() -> Settings:
    ...
    config_root = _discover_config_root(project_root)
    env = _overlay(("IMAP_", "AWS_", "OPENAI_", "PROJECT_", "SOUND_"), _env_file(config_root / "settings.env"))
    database_env_path = Path(os.environ.get(
        "AMAZON_IMAP_BOT_DATABASE_ENV",
        config_root / "database.env",
    )).expanduser()
    database_env = _overlay(("DB_", "ORACLE_WALLET_PASSWORD"), _env_file(database_env_path))
    ...
    return Settings(
        ...
        imap_timeout_seconds=max(1.0, float(env.get("IMAP_TIMEOUT_SECONDS", "5"))),
